...
class BertImgModel(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None,
        ...
        text_output = self.pooler(sequence_output[:, :input_ids.shape[1], :])
        video_output = self.pooler(sequence_output[:, input_ids.shape[1]:, :])

        outputs = (sequence_output, pooled_output, text_output, video_output) + encoder_outputs[1:]  # add hidden_states and attentions if they are here
        return outputs  # sequence_output, pooled_output, (hidden_states), (attentions)
...
class BertCaptioningSentenceLoss(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.loss = CrossEn()
        self.temperature = config.t

    def forward(self, text_output, video_output):
        ...
        logits = 100 * torch.div(
            torch.matmul(text_output, video_output.T),
            self.temperature)

        loss1 = self.loss(logits)
        loss2 = self.loss(logits.T)
...
class BertForImageCaptioning(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.s_loss = BertCaptioningSentenceLoss(config)
    ...
    def encode_forward(self, input_ids, img_feats, attention_mask,
            ...
            is_training=True, encoder_history_states=None, img_feats_f=None):
        img_feats = torch.cat((img_feats, img_feats_f), 1)
        ...
        if is_training:
            ...
            text_output, video_output = outputs[2], outputs[3]
            sentence_loss = self.s_loss(text_output, video_output)
            ...
            loss = masked_loss + self.lamda * sentence_loss
            # loss = masked_loss
    ...
    def prepare_inputs_for_generation(self, curr_ids, past=None):
        ...
        return {'input_ids': input_ids, 'img_feats': img_feats, 'img_feats_f': img_feats_f,
    ...
    def generate(self, img_feats, attention_mask, masked_pos, img_feats_f=None, token_type_ids=None,
        ...
        self.img_seq_len = img_feats.shape[1] + img_feats_f.shape[1]
        ...
        self.img_feats_f = self._expand_for_beams(img_feats_f, num_expand)
