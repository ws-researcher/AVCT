...
def prepare_single_video_frames(vid_path, start_time, end_time, num_frames=32):
    ...
    for i in range(num_frames):
        current_image_path = op.join(data_path, f'{video_id}_{start_time}_{end_time}frame{(i+1):04d}.jpg')
...
def process_video_chunk(item, image_size=56, num_frames=32):
    # line_items = []
    # for item in items:
    vid_id, vid_path, start_time, end_time = item
    images = prepare_single_video_frames(vid_path, start_time, end_time, num_frames)
    if images == None:
        return None
    image_binaries, image_shape = get_image_binaries(images, image_size)
    
    # resolved_data_vid_id, vid_path, _, _ = item
    line_item = [str(vid_id), json.dumps({"class": -1, "width": image_shape[0], "height": image_shape[1], "start_time": start_time, "end_time": end_time})]
...
def main(args):
    dataset_path = f'./datasets/{args.dataset}/0/'
    output_folder = op.join(dataset_path, "frame_tsv")
    ...
    if args.dataset == "MSRVTT":
        ...
        data_path = op.join(dataset_path, "{}frames/").format(args.num_frames)


    data = load_tsv_to_mem(op.join(dataset_path, "{}.img.tsv").format(args.split))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--split", help="train/val",
