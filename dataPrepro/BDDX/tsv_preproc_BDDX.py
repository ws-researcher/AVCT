...
data_vid_name = "datasets/BDDX/raw_videos/{}.mov"
data_vid_id = "datasets/BDDX/raw_videos/{}_{}-{}-{}.mov"
...
def dump_tsv_gt_to_coco_format(caption_tsv_file, outfile):
    ...
    for cap_row  in caption_tsv:
        ...
        for i in range(count):
            ...
            annotations.append(
                        {'image_id': image_id, 'action': action, 'justification': justification, 'caption': action + " " + justification,
                        'id': cap_id})
            cap_id += 1
    ...
    for video_ord, video_info in enumerate(video_list):
        ...
        if True:
            if True:
                ...
                start_time = annotations["sTime"]
                end_time = annotations["eTime"]
                action = annotations['action']
                justification = annotations['justification']

                resolved_data_vid_id = data_vid_id.format(split, start_time, end_time, video_name)
                resolved_data_vid_name = data_vid_name.format(video_name)
                # resolved_data_vid_id = split + '_' + resolved_video_name
                output_captions = []

                output_captions.append({"action": action, "justification": justification, "caption": action + " " + justification})
                ...
                img_label.append([str(resolved_data_vid_id), str(resolved_data_vid_name), start_time, end_time])
