...
def prepare_single_video_frames(vid_path, num_frames=32):
    ...
    filename = ("-").join(vid_path.split('/')[-3:-1])
    vid, _ = os.path.splitext(filename)

    for i in range(num_frames + 1):
        current_image_path = op.join(data_path, f'{vid}-frame{(i+1):04d}.jpg')
...
def main(args):
    ...
    if args.dataset == "MSRVTT":
        data_path = f"datasets/MSRVTT/{args.num_frames}frames/"
    else:
