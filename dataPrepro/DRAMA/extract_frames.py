...
from src.utils.tools import get_gif_last_frame
...
def extract_frame_from_video(video_path, save_frame_path, fps=1, num_frames=-1,
    ...
    if num_frames <= 0 :
        ...
        for frame_idx in range(num_frames):
            image_path = f"{save_frame_path}{(frame_idx+1):04d}.jpg"
        ...
        # lastFrame_cmd_template = "ffmpeg -i {video_path} -filter_complex \"select=\'eq(n,$frame_count)\'\" -vframes 1 {output_frame_path}%04d.png"
        # cur_lastFrame_cmd = lastFrame_cmd_template.format(
        #     video_path=video_path, output_frame_path=save_frame_path)
    ...
    image_path = f"{save_frame_path}{(num_frames + 1):04d}.jpg"
    get_gif_last_frame(video_path, image_path)
...
def extract_frame(video_file_path, save_dir, fps, num_frames, debug=False, corrupt_files=[]):
    filename = ("-").join(video_file_path.split('/')[-3:-1])
    # filename = os.path.basename(video_file_path)
    vid, _ = os.path.splitext(filename)
    frame_name = f"{vid}-frame"
...
def extract_all_frames(video_root_dir, save_dir, fps, num_frames,
    ...
    if num_workers > 0:
        ...
        for idx, d in tqdm(enumerate(videoFiles),
            ...
            extract_frame(d, save_dir, fps=fps, debug=debug, num_frames=num_frames)
            if debug and idx >= 10:
                break
...
def main():
    ...
    parser.add_argument("--video_root_dir", type=str, help="video root dir", default="/usb/Drama_data/combined/")
    parser.add_argument("--save_dir", type=str, help="save frame dir ", default="./datasets/DRAMA/")
    ...
    parser.add_argument("--video_info_tsv", type=str, default="./datasets/DRAMA/test.img.tsv",
