...
def extract_frame_from_video(video_path, save_frame_path, fps=1, num_frames=-1,
    ...
    if num_frames <= 0 :
        extract_cmd_template = "ffmpeg {extra} -i {video_path} -vf fps={fps} {output_frame_path}%06d.jpg"
        
        cur_extract_cmd = extract_cmd_template.format(
        ...
        # split_cmd_template = "ffmpeg -i {video_path} -ss "

        extract_cmd_template = "ffmpeg {extra} -i {video_path} -vf fps={frame_rate} {output_frame_path}%04d.jpg"

        cur_extract_cmd = extract_cmd_template.format(
            extra=extra_args, video_path=video_path, frame_rate=frame_rate, output_frame_path=save_frame_path)
        if not suppress_msg:
            print(cur_extract_cmd)
    try:
        _ = subprocess.run(cur_extract_cmd.split(), stdout=subprocess.PIPE)
...
def extract_frame(video_file_path, save_dir, fps, num_frames, debug=False, corrupt_files=[]):
    start_time, end_time = int(video_file_path[1]), int(video_file_path[2])
    video_file_path = video_file_path[0]
    filename = os.path.basename(video_file_path)
    vid, _ = os.path.splitext(filename)
    frame_name = f"{vid}_{start_time}_{end_time}frame"
    ...
    if launch_extract:
        ...
        # scale=width:height
        extract_frame_from_video(video_file_path, frame_save_path, fps=fps, num_frames=num_frames, start_ts=start_time, end_ts=end_time,
...
def extract_all_frames(video_root_dir, save_dir, fps, num_frames,
    ...
    for _, line_item in enumerate(raw_video_info):
        input_file = line_item[1]
        start_time = line_item[2]
        end_time = line_item[3]
        if os.path.isfile(input_file):
            videoFiles.append((input_file, start_time, end_time))
    if debug:
...
def main():
    ...
    parser.add_argument("--video_root_dir", type=str, help="video root dir", default="./datasets/ActivityNetCaption/videos/")
    parser.add_argument("--save_dir", type=str, help="save frame dir ", default="./datasets/ActivityNetCaption/")
    ...
    parser.add_argument("--video_info_tsv", type=str, default="./datasets/ActivityNetCaption/val.img.tsv",
